...
class NavigationTests : BaseTest() {
    ...
    fun `navigate_to_entree_menu_from_start_order`() {
        ...
        assertEquals(navController.currentDestination?.id, R.id.entreeMenuFragment2)
    }
    ...
    fun `navigate_to_start_order_from_entree_menu`() {
        ...
        entreeMenuScenario.onFragment{ fragment ->
            ...
            // destination
            navController.setCurrentDestination(destId = R.id.entreeMenuFragment2)
            Navigation.setViewNavController(fragment.requireView(), navController)
        }
        // Click the cancel button
        onView(withId(R.id.cancel_button)).perform(click())
        // Check that the destination is correct
        assertEquals(navController.currentDestination?.id, R.id.startOrderFragment2)
    }
    ...
    fun `navigate_to_start_order_from_side_menu`() {
        ...
        sideMenuScenario.onFragment{ fragment ->
            navController.setGraph(R.navigation.mobile_navigation)
            navController.setCurrentDestination(destId = R.id.sideMenuFragment2)
            Navigation.setViewNavController(fragment.requireView(), navController)
        }
        onView(withId(R.id.cancel_button)).perform(click())
        assertEquals(navController.currentDestination?.id, R.id.startOrderFragment2)
    ...
    fun `navigate_to_accompaniment_menu_from_side_menu`() {
        ...
        onView(withId(R.id.next_button)).perform(click())
        assertEquals(navController.currentDestination?.id, R.id.accompanimentMenuFragment2)
    }
    ...
    fun `navigate_to_start_order_from_accompaniment_menu`() {
        ...
        accompanimentMenuScenario.onFragment{ fragment ->
            navController.setGraph(R.navigation.mobile_navigation)
            navController.setCurrentDestination(destId = R.id.accompanimentMenuFragment2)
            Navigation.setViewNavController(fragment.requireView(), navController)
        }
        onView(withId(R.id.cancel_button)).perform(click())
        assertEquals(navController.currentDestination?.id, R.id.startOrderFragment2)
    ...
    fun `navigate_to_checkout_from_accompaniment_menu`() {
        ...
        onView(withId(R.id.next_button)).perform(click())
        assertEquals(navController.currentDestination?.id, R.id.checkoutFragment2)
    }
    ...
    fun `navigate_to_start_order_from_checkout`() {
        ...
        checkoutScenario.onFragment{ fragment ->
            navController.setGraph(R.navigation.mobile_navigation)
            navController.setCurrentDestination(destId = R.id.checkoutFragment2)
            Navigation.setViewNavController(fragment.requireView(), navController)
        }
        onView(withId(R.id.cancel_button)).perform(click())
        assertEquals(navController.currentDestination?.id, R.id.startOrderFragment2)
